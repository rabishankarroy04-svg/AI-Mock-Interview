...
const QuestionSection = ({ mockInterviewQuestion, activeQuestionIndex }) => {
  ...
  // Clean up speech on unmount or question change
  useEffect(() => {
    return () => {
  ...
  return (
    <div className="flex flex-col justify-between p-5 border rounded-lg my-1 bg-secondary">
      ...
      <div className="flex items-center gap-3">
        <Volume2
          className={`cursor-pointer w-6 h-6 ${isPlaying ? "opacity-50 pointer-events-none" : ""}`}
          onClick={() =>
      ...
      <div className="border rounded-lg p-5 bg-blue-100 mt-10 md:block hidden">
        ...
        <h2 className="text-sm text-blue-600 my-2">
          {process.env.NEXT_PUBLIC_QUESTION_NOTE ||
            "Please ensure your face is visible and avoid switching tabs."}
        </h2>
      </div>
    </div>
  );
